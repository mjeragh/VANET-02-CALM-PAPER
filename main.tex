...
\textbf{MASAC achieves the highest entropy} ($1.38$) through its automatic temperature tuning mechanism, which adapts the entropy bonus online. Unlike the entropy-only baseline with a fixed coefficient, SAC's learned temperature successfully prevents mode collapse. However, this high entropy is \emph{undirected}: SAC allocates $34\%$ of actions to Lane\_Right (vs.\ $0.2\%$ in NGSIM) and only $38\%$ to Maintain (vs.\ $55\%$), yielding $25.8\%$ agreement---near the rule-based baseline. This demonstrates that preventing mode collapse is necessary but not sufficient; without demonstration anchoring, the resulting diversity does not resemble human behavior.
...
\begin{enumerate}
    ...
    \item \textbf{MASAC prevents collapse but not human-likeness}: SAC's automatic temperature tuning achieves the highest entropy ($1.38$) but only $25.8\%$ NGSIM agreement---near random. This distinguishes \emph{undirected diversity} (SAC) from \emph{structured diversity} (CALM), and confirms that demonstration anchoring is the critical ingredient.
\end{enumerate}
