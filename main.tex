...
We evaluate CALM against five baselines---DAgger, GAIL, entropy-regularized MADDPG, QMIX-CALM, and Multi-Agent SAC---on a cooperative highway exit coordination task validated against the NGSIM naturalistic driving dataset.
...
CALM introduces two key hyperparameters: the initial KL penalty weight $\lambda_0$ and its exponential decay rate $\delta$, where $\lambda(t) = \lambda_0 \cdot \delta^t$. To assess sensitivity, we sweep $\lambda_0 \in \{0.5, 1.0, 2.0, 5.0\}$ and $\delta \in \{0.99, 0.995, 0.999, 1.0\}$, training each of the 16 configurations with 5 seeds (80 runs total, 300 episodes each). Table~\ref{tab:ablation} reports the last-50-episode average reward.

\begin{table}[t]
\centering
\caption{Hyperparameter sensitivity: last-50-episode average reward across $\lambda_0$ (initial KL weight) and $\delta$ (decay rate). Each cell reports mean $\pm$ std over 5 seeds. Bold denotes the best configuration; the default is marked with $\dagger$.}
\label{tab:ablation}
\footnotesize
...
The results demonstrate that CALM is robust to hyperparameter selection: all 16 configurations achieve rewards between $1.30$ and $1.39$, a variation of less than $7\%$. The default configuration ($\lambda_0 = 2.0$, $\delta = 0.999$) achieves $1.36 \pm 0.04$, ranking second among all configurations. Notably, the best-performing configuration ($\lambda_0 = 0.5$, $\delta = 0.99$) uses a weaker initial constraint with faster decay, suggesting that even light demonstration anchoring suffices to prevent mode collapse. At the other extreme, $\delta = 1.0$ (no decay) maintains a constant KL penalty throughout training and still achieves competitive performance ($1.33$--$1.35$), indicating that the decaying schedule is beneficial but not critical.

Stronger initial constraints ($\lambda_0 = 5.0$) slightly compress the reward range across decay rates ($1.32$--$1.33$), as the dominant KL term leaves less room for reward optimization early in training. This confirms the design intuition behind CALM: moderate $\lambda_0$ with gradual decay provides the best balance between behavioral anchoring and reward maximization.
